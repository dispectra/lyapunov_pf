...
############################### MODIFICATION #################################################################################
import math
from Controller.lyapunov2 import Controller

csv_header = "step,rate,time,speed,throttle,brake,steer,x,y,yaw,idx,ev,elat,eyaw,sp_x,sp_y,sp_yaw,sp_v,sp_curvature"

# path_sim = "./pid_data/cost0/"
# path_sim = "./pid_data/cost1/lamda_0_30/"
# path_sim = "./pid_data/cost2/lamda_0_40/"
path_sim = "./pid_data/cost3/lamda_0_10/"
filename = path_sim + "carla_sim.csv"
...
print("date & time now =", date_log)
# dd/mm/YY H:M:S
dt_string = date_log.strftime("%d-%m-%Y_%H.%M.%S")
# print("date and time =", dt_string)

filename_log = "./data_logging/LogData_" + dt_string +".csv"
#####################
...
def make_carla_settings(args):
    ...
    settings.set(
        ...
        SeedVehicles=0,
        SeedPedestrians=0,
        #PlayerVehicle="/Game/Blueprints/Vehicles/Tesla/Tesla.Tesla_C",
        WeatherId=9,
...
class CarlaGame(object):
    def __init__(self, carla_client, args):
        ...
        ssr = np.load('pid_data/ssr.npy')[-1]
        waypoints_np = np.load('waypoints_interpolated.npy')
        waypoints_np[:, -1] = 0.
        self._conv_rad_to_steer = 180.0 / 70.0 / np.pi
        pid = np.load(path_sim+'the_best_param_history.npy')[-1]

        print(pid)
        print(ssr)
        print('###FLAG1')
        self._brake_gain = np.abs(3.)
        self._controller = Controller(pid[0], pid[1], pid[2],
                            ssr, np.array([-1.*self._brake_gain, 1.]),
        ...
        if self._log:
            print("Logging Enabled")
            self.data_logging = open(filename_log, "w")
            self.data_logging.write(csv_header + '\n')
        ############################
            ...
            while True:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        print(f"Last waypoint index: {self._controller.get_closest_index()}")
                        return
                self._on_loop()
        finally:
            pygame.quit()
    ...
    def _final_project(self, player_measurements, measurements):
        ...
        if now > start_time_threshold:
            ...
            control.brake = max(-u, 0.) / np.abs(self._brake_gain)
            trajectory = self._controller.get_instantaneous_setpoint()
            error = self._controller.get_error()
            ####### ADDED BY DIMAS
            lyap_factor = self._controller.get_lyap_factor()
            ######################

            if self._controller.get_closest_index() >= 34400:
                pygame.quit()
                print(f"Last waypoint index: {self._controller.get_closest_index()}")
                sys.exit("Finish")
            ...
            self._idx += 1
            if self._log:
                message = '{step},{rate},{time},{speed},{throttle},{brake},{steer},{x},{y},{yaw},{idx},{ev},{elat},{eyaw},{sp_x},{sp_y},{sp_yaw},{sp_v},{sp_curvature},{lyap_a},{lyap_b},{lyap_c},{lyap_d},{lyap_e}'
                message = message.format(
                    ...
                    y=y,
                    yaw=yaw,
                    idx=self._controller.get_closest_index(),
                    ev=error[0],
                    elat=error[1],
                    eyaw=error[2],
                    sp_x=trajectory[0],
                    ...
                    lyap_a=lyap_factor[0],
                    lyap_b=lyap_factor[1],
                    lyap_c=lyap_factor[2],
                    lyap_d=lyap_factor[3],
                    lyap_e=lyap_factor[4],
                )
                self.data_logging.write(message + '\n')

        self._last_speed = speed
        if step % 50 == 0:
            print('now: {:.2f} || step: {} || rate: {:.3f} s|| speed: {:.5f} m/s || steer: {:.3f} || throttle: {:.3f} || brake: {:.3f}' \
