...
class Controller(object):
    ...
    def calculate_control_signal(self, dt, x, y, v, yaw):
        ...
        Lh = 3  # Panjang wheelbase (jarak roda depan-belakang)
        k1 = 100
        k2 = 10
        kv = 10**(-10)  # Agar penyebut tidak nol ketika v = 0

        # Small Angle Approximation & tidak tak-terdefinisi utk e_yaw = 0
        ksaa = 0.05
        saa = 1
        if self._e_yaw > ksaa:
            saa = np.sin(self._e_yaw) / self._e_yaw 

        # Sesuai rumus banget
        # a = self._e_yaw * k1 * Lh / (kv + v)
        # b = Lh * self._e_lat * saa

        # Modified
        a = self._e_yaw * k1 / (kv + v)
        b = k2 * self._e_lat * saa
